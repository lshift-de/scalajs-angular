...
import com.greencatsoft.angularjs.{ AngularElement, Config, Controller, Constant, Directive, Factory, Service, Runnable }
...
private[angularjs] object Angular {
  ...
  def constant[A <: Constant[_]](c: Context)(target: c.Expr[A])(implicit tag: c.WeakTypeTag[A]): c.Expr[api.Module] = {
    import c.universe._

    val proxy = newObjectWrapper(c)(target)
    val name = moduleName[A](c)

    c.Expr[api.Module](q"{${c.prefix.tree}.module.constant($name, $proxy)}")
  }

  def constantFromClass[A <: Constant[_]](c: Context)(implicit tag: c.WeakTypeTag[A]): c.Expr[api.Module] = {
    import c.universe._

    val proxy = newClassWrapper(c)
    val name = moduleName[A](c)

    c.Expr[api.Module](q"{${c.prefix.tree}.$$constant($name, $proxy)}")
  }
  ...
  private def moduleName[A <: Service](c: Context)(implicit tag: c.WeakTypeTag[A]): c.universe.Literal = {
    import c.universe.{Constant => SConstant, _}
